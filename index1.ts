...
function processData(args : any, type: String){
  ...
  let retstr = '';
  if(type == 'edit'){
    Object.keys(args).map((key) => setData+=' "'+key +'"'+"='"+args[key]+"' ," );    
    retstr = removeLastChar(setData);
  }else if(type == 'add'){
    let col = '';
    let val = '';
    Object.keys(args).map((key) =>{ col+=' "'+key +'" ,'; val+="'"+args[key]+"' ,";} );
...
const resolvers = {
  ...
  Mutation: {
    addUser:  async (parent: any,{...data}:any, context: any, info: any) => {
      ...
      const instRec = await psqlClient.queryObject("insert into user_tbl "+retstr+"  returning *");
      return instRec.rows;      
    },
    editUser:  async (parent: any,{...data}:any, context: any, info: any) => {
      const args = data.input;      
      let retstr = processData(args,'edit');               
      await psqlClient.queryObject("UPDATE  user_tbl SET "+retstr+" WHERE id='"+data.id+"'");
      const results = await psqlClient.queryObject("select * from user_tbl where id='"+data.id+"';");
      return results.rows;      
    },
    deleteUser:  async (parent: any,{...data}:any, context: any, info: any) => {
                    
      const results = await psqlClient.queryObject("delete from user_tbl where id='"+data.id+"';");
